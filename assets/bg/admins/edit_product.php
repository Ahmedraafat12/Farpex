        ...
        <div class="content-bar">
                ...
                if (isset($_POST["sub_form"])) {
                    ...
                    if (empty($product_title) || 
                        ...
                        if ($conn->query($sql) === TRUE) {
                            echo "<div style='margin:20px; font-size:18px; padding:10px 15px; background-color:#e6fff5;'>لقد تم تعديل المنتج بنجاح</div>";
                        } else {
            ...
            <div class="container-form">
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <textarea name="editorl" id="editorl"><?php echo $row4["disc"] ?></textarea>
                    <textarea style="height: 100px;" name="shipping_info" id="" placeholder="معلومات الشحن والتسليم" ><?php echo $row4["shipping_info"]  ?></textarea>
                    <?php
                            ...
                            for ($i = 0; $i < count($files); $i++) {
                                if ($files[$i] != '.' && $files[$i] != '..') {
                                    ...
                                    if (in_array($extension, $extensions_array)) {
                                        echo "<div id='" . $rm_dot . "' class='item-img'>
                                        <div class='close-btn' onclick='del_img(\"" . $rm_dot . "\", \"" . $files[$i] . "\");'></div>
                                        <img src='$dir_path$files[$i]' style='width: 100%;'>
                                        </div>";
                                        echo "<script>
                                        function del_img(id,imageName) {
                                            document.getElementById(id).style.display = 'none';
                                            var delImgsInput = document.getElementById('del_imgs');
                                            var currentVal = delImgsInput.value;
                                            if (currentVal === 'text') {
                                                delImgsInput.value = imageName;
                                        } 
                                        else 
                                        {
                                            delImgsInput.value += ','+imageName;
                    ...
                    <p style="width:100%;">
                        ...
                        <input style="margin-right:20px;" type="button" class="file-btn" value="اضف صور المنتج" onclick="document.getElementById('files').click()">
                    </p>
                    </div>

                    <input type="hidden" name="del_imgs" id="del_imgs" value="text">
